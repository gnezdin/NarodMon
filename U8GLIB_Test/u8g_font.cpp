/*
  ...
  Высокоуровневый интерфейс шрифтов библиотеки U8G.
*/
...
/*
В структуре данных шрифта из 17 байт доступ к отдельным информационным
элементам шрифта происходит по смещению вместо использования стандатных
полей структуры.
  смещение
  0             формат шрифта
  ...
  10            начало кодировки
  11            конец кодировки
  12            descent 'g'                     отрицательное значение: ниже базовой линии
  13            max ascent для шрифта
  14            min decent для шрифта           отрицательное значение: ниже базовой линии
  15            font xascent
  16            font xdecent                    отрицательное значение: ниже базовой линии
*/

/* Случай использования: какова ширина и высота минимальной прямоугольной области экрана,
   куда вписывается строка s? */
...
/* Случай использования: известен нижний левый срез минимальной прямоугольной области,
   какова корректная позиция x, y для процедуры рисования строки? */
...
/* Случай использования: известна точка базовой линии, возврат минимальной прямоугольной области */
...
/*========================================================================*/
/* низкоуровневый доступ к байту и слову                                  */

/* NOINLINE удалено, потому что это уменьшает код, и возможно будет работать быстрее */
...
/* Вычисляет общую длину шрифта, используется только для создания картинки на google wiki */
size_t u8g_font_GetSize(const void *font)
{
  ...
  p += U8G_FONT_DATA_STRUCT_SIZE;       /* пропуск общей информации о шрифте */  
...
/*========================================================================*/
/* Обработка картинки символа (glyph) */
...
/*
  Находит (с некоторой оптимизацией по скорости) и возвращает указатель на структуру данных глифа.
  Также распаковывает (формат 1) и копирует содержимое структуры данных глифа в структуру u8g.
*/
u8g_glyph_t u8g_GetGlyph(u8g_t *u8g, uint8_t requested_encoding)
{
  ...
  else 
  {
    ...
    if ( requested_encoding >= 65 && pos > 0 )
    {
      ...
      p += U8G_FONT_DATA_STRUCT_SIZE; /* пропуск общей информации шрифта */  
  }
