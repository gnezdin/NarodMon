...
/* Раскомментируйте следующую строку для поддержки дисплеев больше чем 240x240 */
#define U8G_16BIT 1

/* Ширина должна делиться на 8, самая большая ширина 248, если не выбран
   16-битный режим для u8g */
...
//Режим 2-битной яркости каждого бита:
#define PIXELS_PER_BYTE 4
#define BYTES_PER_ROW (WIDTH/PIXELS_PER_BYTE)

/* Закомментируйте следующую строку для генерации более компактного,
   но не безопасного к прерыванию кода */
#define U8G_INTERRUPT_SAFE 1
//Опция для выбора платформы, сделанная по аналогии с __AVR__, ARDUINO, __MSP430__:
#define __BLACKFIN__
...
/* 
  Используйте интерфейс com напрямую на любых системах, если это не AVR или ARDUINO 
*/
...
//Опция управления компиляцией, созданная по аналогии с U8G_WITH_PINLIST:
#define U8G_BLACKFIN
#include <cdefBF538.h>
#include <time.h>
#include "settings.h"
...
/* mspgcc не имеет секций .progmem. Используйте -fdata-sections. */
...
#ifdef __MSP430__
/*
  Указание секции приведет к тому, что MSP-GCC поместит в RAM даже данные констант,
  как минимум для шрифтов. Но поскольку шрифты - константы, то мы не должны задавать
  это вручную - MSP-GCC достаточно умен, чтобы поместить их в память flash.
*/
# undef U8G_SECTION
# define U8G_SECTION(name)
#endif
...
#if defined(__AVR__)
/* U8G_PROGMEM используется в примере XBM */
...
//__XC8 определяет окружение компиляции MPLAB XC8 Compiler (Microchip)
#ifdef __XC8
/* Прототип устройства (device prototype) */
...
struct _u8g_dev_t
{
  u8g_dev_fnptr dev_fn;         /* процедура устройства */
  void *dev_mem;                /* память устройства */
  u8g_com_fnptr com_fn;         /* процедура обмена */
};
...
{
  u8g_uint_t x, y;      /* будет модифицировано */
  uint8_t pixel;        /* будет модифицировано, последовательность точек
                           или значение прозрачности */
  uint8_t dir;          // направление, в котором растет точка
  uint8_t color;			/* цвет или значение индекса, значение red для режима true color */
  //uint8_t hi_color;		/* старший байт для режима 64K color, младший байт в "color", значение green для режима true color */
  //uint8_t blue;			/* значение blue в режиме true color */
};
/* typedef struct _u8g_dev_arg_pixel_t u8g_dev_arg_pixel_t; */ /* forward decl */

/* диапазон для r,g,b: 0..255 */
...
/* раскомментируйте следующую строку для Atmega HW SPI double speed, issue 89 */
...
uint8_t u8g_com_null_fn_(u8g_t *u8g, uint8_t msg, uint8_t arg_val, void *arg_ptr);               /* u8g_com_null.c */

uint8_t u8g_com_std_sw_spi_fn(u8g_t *u8g, uint8_t msg, uint8_t arg_val, void *arg_ptr);	/* требует U8G_WITH_PINLIST */
...
uint8_t u8g_com_blackfin_hw_spi_fn(u8g_t *u8g, uint8_t msg, uint8_t arg_val, void *arg_ptr);      /* u8g_com_blackfin_hw_spi.c */
void blackfin_hw_spi_write_DMA (uint8_t *data, uint32_t datalen);    /* u8g_com_blackfin_hw_spi.c */
bool blackfin_hw_spi_write_DMA_done (void);
...
/* 
  Трансляция зависящих от системы драйверов com к стандартным (generic) именам com
  Сейчас есть следующие generic com драйверы
  U8G_COM_HW_SPI
  U8G_COM_SW_SPI
  U8G_COM_PARALLEL
  U8G_COM_T6963
...
/* Уверен, что должна быть некоторая безумная причина необходимости в этом */
...
#endif
#if defined (__BLACKFIN__)
#define U8G_COM_HW_SPI u8g_com_blackfin_hw_spi_fn
#endif
#ifndef U8G_COM_HW_SPI
#define U8G_COM_HW_SPI u8g_com_null_fn
#define U8G_COM_ST7920_HW_SPI u8g_com_null_fn
...
void u8g_WriteSequence4LTo16GrDevice(u8g_t *u8g, u8g_dev_t *dev, uint16_t cnt, uint8_t *ptr);
void u8g_WriteRawData_to_GrDevice(u8g_t *u8g, u8g_dev_t *dev, uint16_t cnt, uint8_t *ptr);
...
struct _u8g_page_t
{
  u8g_uint_t page_height;     //высота страницы в точках
  u8g_uint_t total_height;    //общая высота в точках
  u8g_uint_t page_y0;         //начальная координата страницы по Y
  u8g_uint_t page_y1;         //конечная  координата страницы по Y
  //uint8_t pagenum;            //номер страницы
};
...
/*===============================================================*/
/* Буфер страницы (page buffer, pb) */
struct _u8g_pb_t
{
  u8g_page_t p;         // структура описания страницы
  u8g_uint_t width;		// ширина страницы по горизонтали
  void *buf;            // графический растр
};
...
name##_buf: видеобуфер строки из WIDTH байт.
name##_pb: буфер страницы.
*/
#define U8G_PB_DEV(name, width, height, page_height, dev_fn, com_fn) \
section ("sdram0") uint8_t name##_buf[BYTES_PER_ROW*ROWS_PER_PAGE] U8G_NOCOMMON ; \
u8g_pb_t name##_pb = { {page_height, height, 0, 0},  width, name##_buf}; \
...
struct _u8g_t
{
  ...
  u8g_dev_t *dev;                      /* первое устройство в цепочке */
  const u8g_pgm_uint8_t *font;         /* обычный фонт для всех текстовых процедур */
  const u8g_pgm_uint8_t *cursor_font;  /* специальный фонт для процедур курсора */
  uint8_t cursor_fg_color, cursor_bg_color;
  uint8_t cursor_encoding;
  uint8_t mode;                        /* режим отображения, один из U8G_MODE_xxx */
  u8g_uint_t cursor_x;
  u8g_uint_t cursor_y;
  u8g_draw_cursor_fn cursor_fn;
  
  //Кеш для информации рисуемого символа шрифта:
  int8_t glyph_dx;      //??
  int8_t glyph_x;       //смещение BBX графики символа по X
  int8_t glyph_y;       //смещение BBX графики символа по Y
  uint8_t glyph_width;  //ширина BBX графики символа
  uint8_t glyph_height; //высота BBX графики символа
  
  //Функция, которая корректирует высоту рисуемого символа
  // в зависимости от установленной вертикальной позиции
  // точки отсчета шрифта (см. setFontPosTop, setFontPosCenter,
...
uint8_t u8g_Begin(u8g_t *u8g);				/* сброс устройства, перевод его в состояние по умолчанию и вызов u8g_UpdateDimension() */
uint8_t u8g_Init(u8g_t *u8g, u8g_dev_t *dev);   /* полезно только если устройство подключено к жестко закодированным портам */
uint8_t u8g_InitComFn(u8g_t *u8g, u8g_dev_t *dev, u8g_com_fnptr com_fn);	/* Эта процедура для любых случаев, если это не Arduino или AVR (например. ARM, но не Due, которая Arduino) */
...
void DrawFilledEllipse (int xc, int yc, int width, int height);
...
void u8g_dev_ssd1322_2bit_prepare_screen(u8g_t *u8g, u8g_dev_t *dev);
...
extern const u8g_fntpgm_uint8_t myfont[3489] U8G_FONT_SECTION("myfont");
